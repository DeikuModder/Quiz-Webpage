...
const loadElements = (counter: number, parentElement: HTMLElement, mainBody: HTMLElement) => {

  const questionIndexTitle: HTMLHeadingElement = document.createElement('h2')
  questionIndexTitle.id = 'questionIndex'
  elementCreator(questionIndexTitle, `Pregunta ${counter + 1} de ${quizData.length}`, parentElement)

  const questionTitle: HTMLHeadingElement  = document.createElement('h3')
  ...
  quizData[counter].options.map(questions => {
    ...
    const buttonChoice: HTMLButtonElement = document.createElement('button')
